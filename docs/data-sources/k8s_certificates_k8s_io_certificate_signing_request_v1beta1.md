...
- `spec` (Block List, Max: 1) The certificate request itself and any additional information. (see [below for nested schema](#nestedblock--spec))
- `status` (Block List, Max: 1) Derived information about the request. (see [below for nested schema](#nestedblock--status))
...
- `extra` (Map of String) Extra information about the requesting user. See user.Info interface for details.
- `groups` (List of String) Group information about the requesting user. See user.Info interface for details.
- `signer_name` (String) Requested signer for the request. It is a qualified name in the form: `scope-hostname.io/name`. If empty, it will be defaulted:
...
- `usages` (List of String) allowedUsages specifies a set of usage contexts the key will be valid for. See: https://tools.ietf.org/html/rfc5280#section-4.2.1.3
...
- `conditions` (Block List) Conditions applied to the request, such as approval or denial. (see [below for nested schema](#nestedblock--status--conditions))
...
- `last_update_time` (String) timestamp for the last update to this condition
