...
page_title: "kubefu_k8s_policy_pod_disruption_budget_v1 Data Source - terraform-provider-kubefu"
subcategory: ""
description: |-
  PodDisruptionBudget is an object to define the max disruption that can be caused to a collection of pods
---

# kubefu_k8s_policy_pod_disruption_budget_v1 (Data Source)
...
- `selector` (Block List, Max: 1) Label query over pods whose evictions are managed by the disruption budget. A null selector will match no pods, while an empty ({}) selector will select all pods within the namespace. (see [below for nested schema](#nestedblock--spec--selector))
- `unhealthy_pod_eviction_policy` (String) UnhealthyPodEvictionPolicy defines the criteria for when unhealthy pods should be considered for eviction. Current implementation considers healthy pods, as pods that have status.conditions item with type="Ready",status="True".
