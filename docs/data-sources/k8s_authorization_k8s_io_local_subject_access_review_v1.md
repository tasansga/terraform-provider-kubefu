...
- `spec` (Block List, Min: 1, Max: 1) Spec holds information about the request being evaluated.  spec.namespace must be equal to the namespace you made the request against.  If empty, it is defaulted. (see [below for nested schema](#nestedblock--spec))

### Optional

- `metadata` (Block List, Max: 1) Standard list metadata. More info: https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata (see [below for nested schema](#nestedblock--metadata))
- `status` (Block List, Max: 1) Status is filled in by the server and indicates whether the request is allowed or not (see [below for nested schema](#nestedblock--status))
...
- `extra` (Map of String) Extra corresponds to the user.Info.GetExtra() method from the authenticator.  Since that is input to the authorizer it needs a reflection here.
- `groups` (List of String) Groups is the groups you're testing for.
- `non_resource_attributes` (Block List, Max: 1) NonResourceAttributes describes information for a non-resource access request (see [below for nested schema](#nestedblock--spec--non_resource_attributes))
- `resource_attributes` (Block List, Max: 1) ResourceAuthorizationAttributes describes information for a resource access request (see [below for nested schema](#nestedblock--spec--resource_attributes))
...
<a id="nestedblock--spec--non_resource_attributes"></a>
...
<a id="nestedblock--spec--resource_attributes"></a>
### Nested Schema for `spec.resource_attributes`

Optional:

- `field_selector` (Block List, Max: 1) fieldSelector describes the limitation on access based on field.  It can only limit access, not broaden it.

This field  is alpha-level. To use this field, you must enable the `AuthorizeWithSelectors` feature gate (disabled by default). (see [below for nested schema](#nestedblock--spec--resource_attributes--field_selector))
- `group` (String) Group is the API Group of the Resource.  "*" means all.
- `label_selector` (Block List, Max: 1) labelSelector describes the limitation on access based on labels.  It can only limit access, not broaden it.

This field  is alpha-level. To use this field, you must enable the `AuthorizeWithSelectors` feature gate (disabled by default). (see [below for nested schema](#nestedblock--spec--resource_attributes--label_selector))
...
<a id="nestedblock--spec--resource_attributes--field_selector"></a>
### Nested Schema for `spec.resource_attributes.field_selector`

Optional:

- `raw_selector` (String) rawSelector is the serialization of a field selector that would be included in a query parameter. Webhook implementations are encouraged to ignore rawSelector. The kube-apiserver's *SubjectAccessReview will parse the rawSelector as long as the requirements are not present.
- `requirements` (Block List) requirements is the parsed interpretation of a field selector. All requirements must be met for a resource instance to match the selector. Webhook implementations should handle requirements, but how to handle them is up to the webhook. Since requirements can only limit the request, it is safe to authorize as unlimited request if the requirements are not understood. (see [below for nested schema](#nestedblock--spec--resource_attributes--field_selector--requirements))

<a id="nestedblock--spec--resource_attributes--field_selector--requirements"></a>
### Nested Schema for `spec.resource_attributes.field_selector.requirements`
...
<a id="nestedblock--spec--resource_attributes--label_selector"></a>
### Nested Schema for `spec.resource_attributes.label_selector`

Optional:

- `raw_selector` (String) rawSelector is the serialization of a field selector that would be included in a query parameter. Webhook implementations are encouraged to ignore rawSelector. The kube-apiserver's *SubjectAccessReview will parse the rawSelector as long as the requirements are not present.
- `requirements` (Block List) requirements is the parsed interpretation of a label selector. All requirements must be met for a resource instance to match the selector. Webhook implementations should handle requirements, but how to handle them is up to the webhook. Since requirements can only limit the request, it is safe to authorize as unlimited request if the requirements are not understood. (see [below for nested schema](#nestedblock--spec--resource_attributes--label_selector--requirements))

<a id="nestedblock--spec--resource_attributes--label_selector--requirements"></a>
### Nested Schema for `spec.resource_attributes.label_selector.requirements`
