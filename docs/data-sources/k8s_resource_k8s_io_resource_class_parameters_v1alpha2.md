...
- `generated_from` (Block List, Max: 1) If this object was created from some other resource, then this links back to that resource. This field is used to find the in-tree representation of the class parameters when the parameter reference of the class refers to some unknown type. (see [below for nested schema](#nestedblock--generated_from))
...
- `named_resources` (Block List, Max: 1) NamedResources describes a resource filter using the named resources model. (see [below for nested schema](#nestedblock--filters--named_resources))

<a id="nestedblock--filters--named_resources"></a>
