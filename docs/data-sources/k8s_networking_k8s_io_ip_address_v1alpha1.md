...
- `parent_ref` (Block List, Max: 1) ParentRef references the resource that an IPAddress is attached to. An IPAddress must reference a parent object. (see [below for nested schema](#nestedblock--spec--parent_ref))
