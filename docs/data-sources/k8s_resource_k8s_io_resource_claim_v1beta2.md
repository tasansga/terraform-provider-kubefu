...
- `spec` (Block List, Max: 1) Spec describes what is being requested and how to configure it. The spec is immutable. (see [below for nested schema](#nestedblock--spec))
...
This is an alpha field and requires enabling the DRADerivedAttributes feature gate. (see [below for nested schema](#nestedblock--spec--devices--requests--exactly--derived_attributes))
...
<a id="nestedblock--spec--devices--requests--exactly--derived_attributes"></a>
### Nested Schema for `spec.devices.requests.exactly.derived_attributes`

Optional:

- `expression` (String) Expression is a CEL expression evaluated against each candidate device. The expression must evaluate to a primitive scalar (string, integer, boolean, or semver) or a list of these scalars ([]string, []int64, []bool, []semver) to act as a virtual grouping key. Any other return type is an error and causes CEL evaluation for the device to fail.
...
- `name` (String) Name is the identifier for this derived attribute, used in constraints.
...
This is an alpha field and requires enabling the DRADerivedAttributes feature gate. (see [below for nested schema](#nestedblock--spec--devices--requests--first_available--derived_attributes))
...
<a id="nestedblock--spec--devices--requests--first_available--derived_attributes"></a>
### Nested Schema for `spec.devices.requests.first_available.derived_attributes`
...
- `skip_node_operations` (List of String) SkipNodeOperations lists node-local resource operations (gRPC calls) that will be skipped for this allocated device when determining whether operations are necessary on the node. If all allocated devices for a driver in a claim skip an operation, that gRPC call will be skipped. It is a copy of the ResourceSlice.spec.skipNodeOperations value at the time when the device was allocated.
