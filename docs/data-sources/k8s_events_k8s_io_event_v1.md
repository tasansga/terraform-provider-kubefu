...
- `related` (Block List, Max: 1) related is the optional secondary object for more complex actions. E.g. when regarding object triggers a creation or deletion of related object. (see [below for nested schema](#nestedblock--related))
...
- `series` (Block List, Max: 1) series is data about the Event series this event represents or nil if it's a singleton Event. (see [below for nested schema](#nestedblock--series))
...
- `count_` (Number) count is the number of occurrences in this series up to the last heartbeat time.
