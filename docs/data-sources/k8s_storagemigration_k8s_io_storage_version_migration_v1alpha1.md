...
- `spec` (Block List, Max: 1) Specification of the migration. (see [below for nested schema](#nestedblock--spec))
- `status` (Block List, Max: 1) Status of the migration. (see [below for nested schema](#nestedblock--status))
...
- `resource` (Block List, Min: 1, Max: 1) The resource that is being migrated. The migrator sends requests to the endpoint serving the resource. Immutable. (see [below for nested schema](#nestedblock--spec--resource))

Optional:

- `continue_token` (String) The token used in the list options to get the next chunk of objects to migrate. When the .status.conditions indicates the migration is "Running", users can use this token to check the progress of the migration.

<a id="nestedblock--spec--resource"></a>
...
- `conditions` (Block List) The latest available observations of the migration's current state. (see [below for nested schema](#nestedblock--status--conditions))
- `resource_version` (String) ResourceVersion to compare with the GC cache for performing the migration. This is the current resource version of given group, version and resource when kube-controller-manager first observes this StorageVersionMigration resource.
