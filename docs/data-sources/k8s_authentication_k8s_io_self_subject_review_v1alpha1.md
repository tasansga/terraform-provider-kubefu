...
- `status` (Block List, Max: 1) Status is filled in by the server with the user attributes. (see [below for nested schema](#nestedblock--status))
...
- `user_info` (Block List, Max: 1) User attributes of the user making this request. (see [below for nested schema](#nestedblock--status--user_info))

<a id="nestedblock--status--user_info"></a>
