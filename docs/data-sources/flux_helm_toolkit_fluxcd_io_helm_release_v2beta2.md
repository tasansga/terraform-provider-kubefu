...
- `spec` (Block List, Max: 1) Spec holds the template for the v1beta2.HelmChartSpec for this HelmRelease. (see [below for nested schema](#nestedblock--spec--chart--spec))
...
- `patches_json6902` (Block List) JSON 6902 patches, defined as inline YAML objects. Deprecated: use Patches instead. (see [below for nested schema](#nestedblock--spec--post_renderers--kustomize--patches_json6902))
...
- `history` (Block List) History holds the history of Helm releases performed for this HelmRelease up to the last successfully completed release. (see [below for nested schema](#nestedblock--status--history))
...
- `last_applied_revision` (String) LastAppliedRevision is the revision of the last successfully applied source. Deprecated: the revision can now be found in the History.
