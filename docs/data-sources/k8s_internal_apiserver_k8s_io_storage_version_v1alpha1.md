...
- `status` (Block List, Max: 1) API server instances report the version they can decode and the version they encode objects to when persisting objects in the backend. (see [below for nested schema](#nestedblock--status))
