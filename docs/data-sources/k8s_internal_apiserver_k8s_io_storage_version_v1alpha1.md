...
- `status` (Block List, Min: 1, Max: 1) API server instances report the version they can decode and the version they encode objects to when persisting objects in the backend. (see [below for nested schema](#nestedblock--status))

### Optional

- `metadata` (Block List, Max: 1) The name is <group>.<resource>. (see [below for nested schema](#nestedblock--metadata))
...
- `conditions` (Block List) The latest available observations of the storageVersion's state. (see [below for nested schema](#nestedblock--status--conditions))
- `storage_versions` (Block List) The reported versions per API server instance. (see [below for nested schema](#nestedblock--status--storage_versions))
...
- `observed_generation` (Number) If set, this represents the .metadata.generation that the condition was set based upon.


<a id="nestedblock--status--storage_versions"></a>
### Nested Schema for `status.storage_versions`

Optional:

- `api_server_id` (String) The ID of the reporting API server.
- `decodable_versions` (List of String) The API server can decode objects encoded in these versions. The encodingVersion must be included in the decodableVersions.
- `encoding_version` (String) The API server encodes the object to this version when persisting it in the backend (e.g., etcd).
- `served_versions` (List of String) The API server can serve these versions. DecodableVersions must include all ServedVersions.
