...
page_title: "kubefu_k8s_scheduling_k8s_io_composite_pod_group_v1alpha3 Data Source - terraform-provider-kubefu"
subcategory: ""
description: |-
...
# kubefu_k8s_scheduling_k8s_io_composite_pod_group_v1alpha3 (Data Source)
...
- `spec` (Block List, Min: 1, Max: 1) spec defines the desired state of the CompositePodGroup. (see [below for nested schema](#nestedblock--spec))
...
- `status` (Block List, Max: 1) status represents the current observed state of the CompositePodGroup. (see [below for nested schema](#nestedblock--status))
...
- `scheduling_policy` (Block List, Min: 1, Max: 1) schedulingPolicy defines the scheduling policy for this instance of the CompositePodGroup. Controllers are expected to fill this field by copying it from a CompositePodGroupTemplate. This field is immutable. (see [below for nested schema](#nestedblock--spec--scheduling_policy))
- `workload_ref` (Block List, Min: 1, Max: 1) workloadRef references an optional CompositePodGroup template within the Workload object that was used to create the CompositePodGroup. This field is required. This field is immutable. (see [below for nested schema](#nestedblock--spec--workload_ref))

Optional:

- `disruption_mode` (Block List, Max: 1) disruptionMode defines the mode in which a given CompositePodGroup can be disrupted. Controllers are expected to fill this field by copying it from a CompositePodGroupTemplate. One of Single, All. Defaults to Single if unset. This field is immutable. (see [below for nested schema](#nestedblock--spec--disruption_mode))
- `parent_composite_pod_group_name` (String) parentCompositePodGroupName contains the name of the parent composite pod group within the same namespace as this composite pod group. It must be a DNS name. If it's nil, then this composite pod group is a root of a workload's hierarchy. This field is immutable.
...
- `priority` (Number) priority is the value of priority of this composite pod group. Various system components use this field to find the priority of the composite pod group. When Priority Admission Controller is enabled, it prevents users from setting this field. The admission controller populates this field from PriorityClassName. The higher the value, the higher the priority. This field is immutable.
...
- `gang` (Block List, Max: 1) gang specifies that the groups of this composite group should be scheduled using all-or-nothing semantics. (see [below for nested schema](#nestedblock--spec--scheduling_policy--gang))
...
- `topology` (Block List) topology defines the topology constraints for the composite pod group. Currently only a single topology constraint can be specified. This may change in the future. (see [below for nested schema](#nestedblock--spec--scheduling_constraints--topology))
...
- `conditions` (Block List) conditions represent the latest observations of the CompositePodGroup's state.
...
Known reasons for the DisruptionTarget condition: - "PreemptionByScheduler": The CompositePodGroup was targeted by the scheduler's preemption loop
  to free up capacity for higher-priority preemptors. (see [below for nested schema](#nestedblock--status--conditions))
