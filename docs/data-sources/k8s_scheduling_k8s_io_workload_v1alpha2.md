...
page_title: "kubefu_k8s_scheduling_k8s_io_workload_v1alpha2 Data Source - terraform-provider-kubefu"
subcategory: ""
description: |-
...
# kubefu_k8s_scheduling_k8s_io_workload_v1alpha2 (Data Source)
...
- `pod_group_templates` (Block List, Min: 1) PodGroupTemplates is the list of templates that make up the Workload. The maximum number of templates is 8. This field is immutable. (see [below for nested schema](#nestedblock--spec--pod_group_templates))
...
- `disruption_mode` (String) DisruptionMode defines the mode in which a given PodGroup can be disrupted. One of Pod, PodGroup. This field is available only when the WorkloadAwarePreemption feature gate is enabled.
- `priority` (Number) Priority is the value of priority of pod groups created from this template. Various system components use this field to find the priority of the pod group. When Priority Admission Controller is enabled, it prevents users from setting this field. The admission controller populates this field from PriorityClassName. The higher the value, the higher the priority. This field is available only when the WorkloadAwarePreemption feature gate is enabled.
...
- `gang` (Block List, Max: 1) Gang specifies that the pods in this group should be scheduled using all-or-nothing semantics. (see [below for nested schema](#nestedblock--spec--pod_group_templates--scheduling_policy--gang))
