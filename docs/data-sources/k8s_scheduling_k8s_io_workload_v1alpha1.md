...
- `metadata` (Block List, Max: 1) Standard object's metadata. Name must be a DNS subdomain. (see [below for nested schema](#nestedblock--metadata))
...
- `pod_groups` (Block List, Min: 1) PodGroups is the list of pod groups that make up the Workload. The maximum number of pod groups is 8. This field is immutable. (see [below for nested schema](#nestedblock--spec--pod_groups))

Optional:

- `controller_ref` (Block List, Max: 1) ControllerRef is an optional reference to the controlling object, such as a Deployment or Job. This field is intended for use by tools like CLIs to provide a link back to the original workload definition. When set, it cannot be changed. (see [below for nested schema](#nestedblock--spec--controller_ref))

<a id="nestedblock--spec--pod_groups"></a>
### Nested Schema for `spec.pod_groups`
...
- `policy` (Block List, Min: 1, Max: 1) Policy defines the scheduling policy for this PodGroup. (see [below for nested schema](#nestedblock--spec--pod_groups--policy))

<a id="nestedblock--spec--pod_groups--policy"></a>
### Nested Schema for `spec.pod_groups.policy`
...
- `gang` (Block List, Max: 1) Gang specifies that the pods in this group should be scheduled using all-or-nothing semantics. (see [below for nested schema](#nestedblock--spec--pod_groups--policy--gang))

<a id="nestedblock--spec--pod_groups--policy--gang"></a>
### Nested Schema for `spec.pod_groups.policy.gang`
