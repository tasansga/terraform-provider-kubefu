...
- `exempt` (Block List, Max: 1) `exempt` specifies how requests are handled for an exempt priority level. This field MUST be empty if `type` is `"Limited"`. This field MAY be non-empty if `type` is `"Exempt"`. If empty and `type` is `"Exempt"` then the default values for `ExemptPriorityLevelConfiguration` apply. (see [below for nested schema](#nestedblock--spec--exempt))
...
<a id="nestedblock--spec--exempt"></a>
