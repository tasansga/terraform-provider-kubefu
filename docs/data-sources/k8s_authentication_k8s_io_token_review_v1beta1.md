...
page_title: "kubefu_k8s_authentication_k8s_io_token_review_v1beta1 Data Source - terraform-provider-kubefu"
subcategory: ""
description: |-
  TokenReview attempts to authenticate a token to a known user. Note: TokenReview requests may be cached by the webhook token authenticator plugin in the kube-apiserver.
---

# kubefu_k8s_authentication_k8s_io_token_review_v1beta1 (Data Source)
...
- `status` (Block List, Max: 1) Status is filled in by the server and indicates whether the request can be authenticated. (see [below for nested schema](#nestedblock--status))
...
- `audiences` (List of String) Audiences is a list of the identifiers that the resource server presented with the token identifies as. Audience-aware token authenticators will verify that the token was intended for at least one of the audiences in this list. If no audiences are provided, the audience will default to the audience of the Kubernetes apiserver.
...
- `user` (Block List, Max: 1) User is the UserInfo associated with the provided token. (see [below for nested schema](#nestedblock--status--user))

<a id="nestedblock--status--user"></a>
### Nested Schema for `status.user`

Optional:

- `extra` (Map of String) Any additional information provided by the authenticator.
