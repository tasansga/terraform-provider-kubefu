...
page_title: "kubefu_k8s_resource_k8s_io_device_taint_rule_v1beta2 Data Source - terraform-provider-kubefu"
subcategory: ""
description: |-
  DeviceTaintRule adds one taint to all devices which match the selector. This has the same effect as if the taint was specified directly in the ResourceSlice by the DRA driver.
---

# kubefu_k8s_resource_k8s_io_device_taint_rule_v1beta2 (Data Source)
...
- `device_selector` (Block List, Max: 1) DeviceSelector defines which device(s) the taint is applied to. All selector criteria must be satisfied for a device to match. The empty selector matches all devices. Without a selector, no devices are matches. (see [below for nested schema](#nestedblock--spec--device_selector))
...
- `effect` (String) The effect of the taint on claims that do not tolerate the taint and through such claims on the pods using them.
...
- `time_added` (String) TimeAdded represents the time at which the taint was added or (only in a DeviceTaintRule) the effect was modified. Added automatically during create or update if not set.
