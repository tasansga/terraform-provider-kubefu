...
func DataSources(versions Versions) map[string]*schema.Resource {
	result := make(map[string]*schema.Resource, 336)
	{
		...
		configured := versionpkg.NormalizeList(versions.versionFor("cert_manager"))
		if len(configured) > 0 {
	...
	}
	{
		...
		warnings := make([]string, 0, 2)
		configured := versionpkg.NormalizeList(versions.versionFor("flux"))
		if len(configured) > 0 {
	...
	}
	{
		...
		warnings := make([]string, 0, 2)
		configured := versionpkg.NormalizeList(versions.versionFor("kustomize"))
		if len(configured) > 0 {
	...
	}
	{
		...
		warnings := make([]string, 0, 2)
		configured := versionpkg.NormalizeList(versions.versionFor("prometheus_operator"))
		if len(configured) > 0 {
